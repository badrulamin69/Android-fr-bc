...
public class BookIssueController {
    ...
    public ResponseEntity<BookIssueResponse> create(@RequestBody BookIssueRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.issueBook(request));
    }
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ADMIN', 'LIBRARIAN', 'STAFF', 'STUDENT')")
    public ResponseEntity<BookIssueResponse> getById(@PathVariable Long id) {
        return ResponseEntity.ok(service.getById(id));
    }
    @GetMapping
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ADMIN', 'LIBRARIAN', 'STAFF', 'STUDENT')")
    public ResponseEntity<Page<BookIssueResponse>> getAll(
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ADMIN', 'LIBRARIAN', 'STAFF')")
    public ResponseEntity<BookIssueResponse> update(@PathVariable Long id, @RequestBody BookIssueRequest request) {
        return ResponseEntity.ok(service.issueBook(request));
    }
    @DeleteMapping("/{id}")
