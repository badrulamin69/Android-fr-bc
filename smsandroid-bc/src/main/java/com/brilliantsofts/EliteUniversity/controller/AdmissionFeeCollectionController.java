...
public class AdmissionFeeCollectionController {
    ...
    public ResponseEntity<AdmissionFeeCollectionResponse> create(@RequestBody AdmissionFeeCollectionRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(request));
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ADMIN', 'ADMISSION_OFFICER')")
    public ResponseEntity<AdmissionFeeCollectionResponse> getById(@PathVariable Long id) {
        return ResponseEntity.ok(service.getById(id));
    }

    @GetMapping
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ADMIN', 'ADMISSION_OFFICER')")
    public ResponseEntity<Page<AdmissionFeeCollectionResponse>> getAll(
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ADMIN', 'ADMISSION_OFFICER')")
    public ResponseEntity<AdmissionFeeCollectionResponse> update(@PathVariable Long id, @RequestBody AdmissionFeeCollectionRequest request) {
